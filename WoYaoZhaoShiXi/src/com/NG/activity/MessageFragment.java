...
public class MessageFragment extends Fragment implements IXListViewListener {
	...
	private Handler updateHandler = new Handler() {
		...
		public void handleMessage(Message message) {
			switch(message.what){
			case DOWNLOAD_WRONG:
				Toast.makeText(mContext, "请求失败，请检查您的网络",
	...
	class LoadUpdateData implements Runnable {
		...
		public void run() {
			...
			try {
				...
				current_time = d.getTime() / 1000;
				//current_time = 1400745135;
				String path = "http://211.155.86.159/online/info/get_message?startTime="
						+ 0
						+ "&endTime="
						+ current_time
				...
				updateList = mMessageLoader.parserMovieJson(path);
				
				updateHandler.sendEmptyMessage(DOWNLOAD_OK);
				
				proDialog.dismiss();

			} catch (Exception e) {
				// TODO Auto-generated catch block
				updateHandler.sendEmptyMessage(DOWNLOAD_WRONG);
				e.printStackTrace();
			}
		}
